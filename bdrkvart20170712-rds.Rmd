...
setwd("~/Git-work/bdr_rap/data")
filnavn <- "avid.RDS"

RegData <- readRDS(filnavn)
...
RegData$regValg[RegData$ValgtData2 == "Overflytting"] <- 4
...
RegData$SykehusKode[RegData$SykehusNavn=="Haugesund sjukehus"] <- 2
RegData$SykehusKode[RegData$SykehusNavn=="Sykehuset i Vestfold, Tønsberg"] <- 3
RegData$SykehusKode[RegData$SykehusNavn=="Nordlandssykehuset HF avd Bodø, somatikk"] <- 4
RegData$SykehusKode[RegData$SykehusNavn=="St. Olavs Hospital"] <- 5
RegData$SykehusKode[RegData$SykehusNavn=="Ålesund sjukehus"] <- 6
RegData$SykehusKode[RegData$SykehusNavn=="Sykehuset Levanger"] <- 7
...
navn <- c("HbA1c", "Insulinsjokk", "DKA", "?ye us.", "Infiltrater", "TSH", "C?liaki", "Fysisk aktivitetet",
          "LDL", "Urin us.")
...
dka1 <- dka %>%
    filter(grepl("F?rstegangsregistrering", inn_Type))
dkaSum <- dim(dka1)[1]
...
komNavn <- c("DKA ved\n f?rstegangsregistrering", "Ketoacidose", "Insulinsjokk")
