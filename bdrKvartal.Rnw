...
\begin{center}
\textbf{\huge Kvartalsrapport for \Sexpr{sykNavn}}\linebreak
\textbf{\Sexpr{tidRom}}
\end{center}

\section*{Antall førstegangsregistreringer og årskontroll}
\label{sec:orgdc9a892}
...
## gg.test <- ggplot(data=hbaLokal, aes(fill=kjonn, x=AlderKat3, y=Mean)) +
##     geom_bar(position = position_dodge(0.9), stat = "identity", na.rm = TRUE) +
##     coord_cartesian(ylim = c(5,10))

## gg.test1 <- ggplot(data=hbaLokal, aes(fill=kjonn, x=AlderKat3, y=Mean)) +
##     geom_point(position = position_dodge(0.9), stat = "identity", na.rm = TRUE) +
##     ylim(5, 10)
...
\label{sec:orgac1a35d}
NB! Fint om dere kan gi riktig navn til variablene. Navnene er bare pluket nesten som
de er fra databasen \linebreak

<<Plot3,echo=FALSE, message=FALSE, warning=FALSE, fig.height=3.5, fig.align='center', dev='pdf', fig.path="graphics/bdr">>=
## white space og NA
trimNJ <- function(x) {
...
\label{sec:orgaad3a95}

Alle med  lab-BiKarbonat < 15.00 eller/og lab-pH < 7.30 i førstegangsreg. Det står
\textbf{"DKA ved årskontroll"} i figuren sendt til meg, men vet ikke hvilken variabel den
skal hentes fra. Ellers \textbf{Ketoacidose} og \textbf{Insulinsjokk} er totalen fra
und-ketoacidose-ant og und-inssjokk-ant \linebreak
